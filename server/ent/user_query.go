...
func (_q *UserQuery) QueryCards() *CardQuery {
...
func (_q *UserQuery) QueryReviewLogs() *ReviewLogQuery {
...
func (_q *UserQuery) WithReviewLogs(opts ...func(*ReviewLogQuery)) *UserQuery {
	query := (&ReviewLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withReviewLogs; query != nil {
		if err := _q.loadReviewLogs(ctx, query, nodes,
			func(n *User) { n.Edges.ReviewLogs = []*ReviewLog{} },
			func(n *User, e *ReviewLog) { n.Edges.ReviewLogs = append(n.Edges.ReviewLogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadDecks(ctx context.Context, query *DeckQuery, nodes []*User, init func(*User), assign func(*User, *Deck)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(deck.FieldUserID)
	}
...
func (_q *UserQuery) loadReviewLogs(ctx context.Context, query *ReviewLogQuery, nodes []*User, init func(*User), assign func(*User, *ReviewLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(reviewlog.FieldUserID)
	}
