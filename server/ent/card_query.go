...
func (_q *CardQuery) QueryUser() *UserQuery {
...
func (_q *CardQuery) QueryDeck() *DeckQuery {
...
func (_q *CardQuery) QueryReviewLogs() *ReviewLogQuery {
	query := (&ReviewLogClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *CardQuery) WithUser(opts ...func(*UserQuery)) *CardQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *CardQuery) WithDeck(opts ...func(*DeckQuery)) *CardQuery {
	query := (&DeckClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CardQuery) WithReviewLogs(opts ...func(*ReviewLogQuery)) *CardQuery {
	query := (&ReviewLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	...
	if query := _q.withReviewLogs; query != nil {
		if err := _q.loadReviewLogs(ctx, query, nodes,
			func(n *Card) { n.Edges.ReviewLogs = []*ReviewLog{} },
			func(n *Card, e *ReviewLog) { n.Edges.ReviewLogs = append(n.Edges.ReviewLogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CardQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Card, init func(*Card), assign func(*Card, *User)) error {
...
func (_q *CardQuery) loadDeck(ctx context.Context, query *DeckQuery, nodes []*Card, init func(*Card), assign func(*Card, *Deck)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Card)
	for i := range nodes {
		fk := nodes[i].DeckID
...
func (_q *CardQuery) loadReviewLogs(ctx context.Context, query *ReviewLogQuery, nodes []*Card, init func(*Card), assign func(*Card, *ReviewLog)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(reviewlog.FieldCardID)
	}
	query.Where(predicate.ReviewLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(card.ReviewLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *CardQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(card.FieldUserID)
		}
		if _q.withDeck != nil {
